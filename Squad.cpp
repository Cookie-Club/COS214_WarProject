...
void Squad::attack() {
    ...
    for (; it != members.end(); ++it) {
        switch (((TeamMembers *) (*it))->getType()) {
            case infantry: // Can change ration consumption based on cell type
                rationsConsumed += ((Infantry *) (*it))->getRationConsumption();
                break;
            case tank: // Can change fuel consumption based on cell type
...
int Squad::getTotalHp() {
    ...
    for (auto i = members.begin(); i != members.end(); ++i) {
        hp += members->operator[](i)->getHealthpoints();
    }
    return hp;
}
